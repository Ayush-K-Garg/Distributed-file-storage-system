...
int main()
{
    ...
    std::cout << "Universal Metadata Server running on port " << metaPort << "..." << std::endl;
    while (true)
    {
        ...
        {
            std::lock_guard<std::mutex> lock(globalMtx);
            std::string resp = "\n--- Network Status: Active Storage Nodes ---\n";
            if (liveNodes.empty())
            {
                resp += "No nodes online.\n";
            }
            else
            {
                for (auto const &[port, info] : liveNodes)
                {
                    resp += " > Node [" + info.ip + ":" + std::to_string(port) + "]";
                    resp += " | Status: ONLINE | Last Heartbeat: " + getTimeAgo(info.last_seen) + "\n";
                }
            }
            resp += "-------------------------------------------\n";
            send(client, resp.c_str(), (int)resp.size() + 1, 0);
        ...
        {
            std::lock_guard<std::mutex> lock(globalMtx);
            std::string fileList = "--- Registered Files in Cloud ---\n";
            ...
            {
                for (auto const &[filename, _] : metadata)
                {
                    fileList += filename + "\n";
                }
            }
            // Sending the list as a single string back to the client
            send(client, fileList.c_str(), (int)fileList.size() + 1, 0);
        }
